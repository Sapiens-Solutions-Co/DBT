{% macro greenplum__create_table_as(temporary, relation, sql) -%}
  ...
  {%- set appendonly = config.get('appendonly', default=none) -%}
  {%- set appendoptimized = config.get('appendoptimized', default=appendonly) -%}
  {%- set orientation = config.get('orientation', default=none) -%}
  {%- set compresstype = config.get('compresstype', default=none) -%}
  {%- set compresslevel = config.get('compresslevel', default=none) -%}
  {%- set blocksize = config.get('blocksize', default=none) -%}
  ...
  {% if is_partition and not temporary and fields_string %}
    {# CREATING TABLE #}
  ...
  {% elif is_partition and not temporary and not fields_string %}
    {# Create a temporary table with a modified identifier #}
    {% set temp_relation = api.Relation.create(
        identifier=relation.identifier ~ '__dbt_struct',
        schema='pg_temp',
        type='table',
        temporary=True
    ) %}    
    {# Create a temporary table to infer the structure #}
    ...
    CREATE TABLE {{ relation }} (LIKE {{ temp_relation }})
    {{ storage_parameters(appendoptimized, blocksize, orientation, compresstype, compresslevel) }}
    {{ distribution(distributed_by, distributed_replicated) }}
    {{ partitions(raw_partition, partition_type, partition_column,
                  default_partition_name, partition_start, partition_end,
    ...
    INSERT INTO {{ relation }} SELECT * FROM {{ temp_relation }};
